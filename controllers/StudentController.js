...
export const createStudent = async (req, res) => {
  const {
    ...
    bacYear,
    address,
    sendCredsInMail = false,
  } = req.body;

  if (!/^[0-9]+$/.test(cin) || cin.length < 8) {
    return res
      .status(400)
      .json({ message: "CIN must be a valid number with at least 8 digits." });
  }
  ...
  try {
    ...
    for (const item of data) {
      ...
      if (existingUser || existingStudent) {
        existingStudent
          ? console.warn(`Skipping row ${index}: Student already exists.`)
          : console.warn(`Skipping row ${index}: Student already exists.`);
        continue;
      }
    ...
    if (users.length != 0) {
      ...
      res
        .status(400)
        .json({ message: "no user created plz check your datafile" });
    }
  } catch (error) {
    // Roll back the transaction if an error occurs
    await session.abortTransaction();
...
export const createCV = async (req, res) => {
  try {
    ...
    if (!studentId) {
      return res
        .status(400)
        .json({ message: "Student ID is not available in the token." });
    }
    ...
    res
      .status(201)
      .json({ model: savedCV, message: "CV created successfully" });
  } catch (error) {
...
export const getStudentById = async (req, res) => {
  ...
  try {
    const student = await Student.findById(id).populate("user", "email phone");
    if (!student) {
      return res.status(404).json({ message: "Student not found." });
    }
    res.status(200).json({
...
export const deleteStudent = async (req, res) => {
  ...
  try {
    ...
    if (typeof force !== "boolean") {
      return res
        .status(400)
        .json({ message: "Invalid value for 'force'. It must be a boolean." });
    }
